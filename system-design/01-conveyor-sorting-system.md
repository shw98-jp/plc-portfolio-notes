...
GX Works2を使用して作成した、金属・非金属製品の仕分けコンベアシステムです。
...
コンベアで搬送される製品が金属か非金属かを判定し、製品が仕分け位置に到着するとコンベアを停止させます。
...
- 製品が仕分け位置に到着すると、コンベアを一時停止する。
- 仕分け装置が製品を排出すると、コンベアを再運転する。
...
<div class="operation-flow" aria-label="設備構成および動作の流れ">
  ...
  <div class="flow-item">
    ...
    <h3>仕分け位置センサー</h3>
    <p><code>X4 ON</code>で<code>Y0 OFF</code>となり、コンベアを停止します。</p>
  </div>
  ...
  <div class="flow-item flow-end">
    ...
    <h3>仕分け完了</h3>
    <p>製品がX4から離れると、<code>RST M10</code> / <code>RST M20</code>を実行し、<code>Y0 ON</code>でコンベアを再運転します。</p>
  </div>
</div>
...
| アドレス | デバイス名 | 用途 |
| --- | --- | --- |
| X0 | START_PB | コンベア始動ボタン |
| X1 | STOP_PB | コンベア停止ボタン |
| X2 | EMG_STOP | 非常停止ボタン |
| X3 | METAL_SENSOR | 金属製品検出センサー |
| X4 | SORT_POSITION_SENSOR | 製品の仕分け位置への到着検出 |

### 出力デバイス

| アドレス | デバイス名 | 用途 |
| --- | --- | --- |
| Y0 | CONVEYOR_MOTOR | コンベアモーター運転 |
...
M0 自己保持</code></pre>
  ...
  <details class="process-step">
    <summary><span class="step-label">4.2</span><span class="step-title">コンベアモーター制御</span></summary>
    <div class="step-body">
      <p>次の条件がすべて成立すると、Y0のコンベアモーターが動作します。</p>
      <pre><code>M0 ON
+
X2 OFF
+
X4 OFF
   ↓
Y0 ON</code></pre>
      <p>製品が仕分け位置に到着してX4がONになると、<code>[/X4]</code>接点が開き、Y0がOFFになってコンベアが停止します。</p>
      <pre><code>X4 ON
   ↓
Y0 OFF
   ↓
コンベア停止</code></pre>
      <figure class="article-image">
        <img src="{{ '/assets/images/system-design/01-conveyor-sorting-system/02_conveyor_motor_control.png' | relative_url }}?v={{ page_cache_bust }}" alt="コンベアモーター制御回路">
        <figcaption>02. コンベアモーター制御回路</figcaption>
      </figure>
    </div>
  </details>
  ...
  <details class="process-step">
    ...
    <div class="step-body">
      <p>コンベア運転中にX3の金属検出センサーがONになると、<code>SET M10</code>命令を実行します。</p>
      <pre><code>M0 ON
+
X3 ON
   ↓
SET M10</code></pre>
      <p>製品がX3センサーを通過してX3がOFFになっても、M10はON状態を維持します。</p>
...
Bボックスへ仕分け</code></pre>
  ...
  <details class="process-step">
    <summary><span class="step-label">4.5</span><span class="step-title">仕分け作業中の状態保持</span></summary>
    <div class="step-body">
...
SET M20</code></pre>
      ...
      <figure class="article-image">
        <img src="{{ '/assets/images/system-design/01-conveyor-sorting-system/05_sorting_active.png' | relative_url }}?v={{ page_cache_bust }}" alt="仕分け作業中の状態保持回路">
        <figcaption>05. 仕分け作業中の状態保持回路</figcaption>
      </figure>
    </div>
  </details>
  ...
  <details class="process-step">
    ...
    <div class="step-body">
      <p>仕分け装置が製品を押し出すと、製品がX4センサーから離れ、X4がOFFになります。</p>
...
RST M20</code></pre>
      ...
      <p>初期化が完了すると、X4がOFF状態であるためY0が再びONになり、コンベアが再運転します。</p>
      <figure class="article-image">
        <img src="{{ '/assets/images/system-design/01-conveyor-sorting-system/06_sorting_reset.png' | relative_url }}?v={{ page_cache_bust }}" alt="仕分け完了および状態の初期化回路">
        <figcaption>06. 仕分け完了および状態の初期化回路</figcaption>
      </figure>
    </div>
  </details>
</div>
